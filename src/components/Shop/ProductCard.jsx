      ...
      >
        ...
        >
          <div className=" h-full w-full  rounded-xl overflow-hidden border shadow pb-3 bg-white/80  backdrop-blur-md  ">
            <Link
              to={!props.forSeller && `/product/${props.id}`}
              state={{ id: props.id }}
            >
            ...
            <Link to={"/seller/editProduct"} state={{ id: props.id }}>
              <div className="w-full justify-center flex items-center ">
                <button
                  className="btn btn-sm h-9 mb-1 w-11/12 rounded-xl bg-green-500 text-gray-500 hover:bg-gray-900 transition-all duration-300 ease-in-out font-semibold text-sm "
                  onClick={!props.forSeller ? handleBuy : handleEdit}
                >
            ...
            <div className="w-full justify-center flex items-center ">
              <button
                onClick={!props.forSeller ? handleAddCart : hadleDelete}
                className={`btn btn-sm py-1 w-11/12 rounded-xl border border-gray-400  bg-transparent hover:text-white hover:bg-gray-900 transition-all duration-500 ease-in-out font-semibold text-sm`}
              >
